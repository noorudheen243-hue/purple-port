...
const FinancialReports = () => {
    ...
    const [reportType, setReportType] = useState('INCOME_AND_EXPENDITURE');
    const statementPrintRef = useRef<HTMLDivElement>(null);

    const reportOptions = [
        { id: 'INCOME_AND_EXPENDITURE', label: 'Income & Expenditure Statement' },
        { id: 'STAFF_SALARY', label: 'Staff Salary Statements' },
        { id: 'EXPENDITURE', label: 'Expenditure Statements' },
        { id: 'INCOME', label: 'Income Statements' }
    ];

    const { data: reportData, isLoading } = useQuery({
        queryKey: ['financial-reports', startDate, endDate, reportType],
        queryFn: async () => {
            return (await api.post('/accounting/unified/reports/financial', {
    ...
    const handleDownloadPDF = async () => {
        if (statementPrintRef.current) {
            ...
            try {
                ...
                pdf.save(`Financial_Report_${reportType}_${format(new Date(), 'yyyyMMdd')}.pdf`);
            } finally {
            ...
            <div className="lg:col-span-4 space-y-6">
                <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 shadow-sm sticky top-8">
                    <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-3">
                        <Filter className="w-5 h-5 text-purple-600" /> Report Configuration
                    </h2>
            ...
            <div className="lg:col-span-8">
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(reportType === 'INCOME_AND_EXPENDITURE' || reportType === 'INCOME') && (
                            <div className="bg-emerald-50 border-2 border-emerald-100 p-6 rounded-[2.5rem]">
                                ...
                                <p className="text-2xl font-black text-emerald-900">{formatCurrency(reportData?.totalIncome || 0)}</p>
                            </div>
                        )}
                        {(reportType === 'INCOME_AND_EXPENDITURE' || reportType === 'EXPENDITURE' || reportType === 'STAFF_SALARY') && (
                            <div className="bg-rose-50 border-2 border-rose-100 p-6 rounded-[2.5rem]">
                                ...
                                <p className="text-2xl font-black text-rose-900">{formatCurrency(reportData?.totalExpense || 0)}</p>
                            </div>
                        )}
                        {reportType === 'INCOME_AND_EXPENDITURE' && (
                            <div className="bg-slate-900 p-6 rounded-[2.5rem] md:col-span-2 text-white">
                                <div className="flex justify-between items-start mb-2">
                                    <BarChart3 className="w-5 h-5 text-purple-400" />
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Net Value</span>
                                </div>
                                <p className="text-2xl font-black">{formatCurrency(reportData?.netTotal || 0)}</p>
                            </div>
                        )}
                    </div>

                    {/* Statement Table */}
                    <div className="bg-white rounded-[3rem] border-2 border-slate-100 shadow-xl overflow-hidden min-h-[500px]" ref={statementPrintRef}>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100">
                                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date</th>
                                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ledger</th>
                                    ...
                                    {reportData?.transactions?.map((t: any) => (
                                        <tr key={t.id} className="hover:bg-slate-50 transition-colors group">
                                            ...
                                            <td className="px-8 py-5">
                                                <div className="font-bold text-slate-900 text-sm">{t.ledger_name}</div>
                                            </td>
                                            <td className="px-8 py-5">
                                            ...
                                            <td className={`px-8 py-5 text-right font-bold text-sm ${t.transaction_type === 'INCOME' ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                {formatCurrency(t.amount)}
                                            </td>
                                        </tr>
                                    ))}

                                    {(!reportData?.transactions || reportData.transactions.length === 0) && !isLoading && (
                                        <tr>
                                            <td colSpan={4} className="px-8 py-20 text-center text-slate-400 font-medium">
                                                No transactions recorded within this period for the selected report.
                                            </td>
                                        </tr>
                                    )}
