...
try {
    ...
    $buildServer = Invoke-SSHCommand -SessionId $s.SessionId -Command "cd /var/www/purple-port/server && npm run build 2>&1" -Timeout 300
    Write-Host $buildServer.Output
    ...
    Write-Host "`n✅ FULL STACK DEPLOYED" -ForegroundColor Green
