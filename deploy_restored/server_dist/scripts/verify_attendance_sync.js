...
function verifySync() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const deviceInfo = yield biometric_service_1.biometricControl.getDeviceInfo();
            ...
            const rawLogs = yield biometric_service_1.biometricControl.getAttendanceLogs();
            console.log(`Fetched ${rawLogs.length} raw logs.\n`);
            // Get all staff profiles to map device IDs
            const staffProfiles = yield db.staffProfile.findMany({
                include: { user: true }
            });
            ...
            rawLogs.forEach((log) => {
                ...
                if (!dateRawLogs[dateKey])
                    dateRawLogs[dateKey] = {};
                if (!dateRawLogs[dateKey][staffNo])
                    dateRawLogs[dateKey][staffNo] = { punches: [] };
            ...
            for (const dateKeyStr of dateKeys) {
                ...
                const dbRecords = yield db.attendanceRecord.findMany({
                ...
                for (const staffNo of Object.keys(rawDataForDate)) {
                    ...
                    const fmt = (d) => d ? d.toISOString().substr(11, 8) : 'N/A';
