...
// ──────────────────────────────────────────────────────────────────────────────
// ENDPOINT 8: DELETE /api/backup/delete/:filename
// Permanently removes a backup file from the local disk.
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// ENDPOINT 9: POST /api/backup/delete-multiple
// Permanently removes multiple backup files from the local disk.
// ──────────────────────────────────────────────────────────────────────────────
const deleteMultipleBackups = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const filename of filenames) {
            try {
                ...
                results.failed.push({ filename, error: err.message || 'Delete failed' });
            }
        }
        console.log(`[Backup] Bulk delete complete. Deleted: ${results.deleted.length}, Failed: ${results.failed.length}`);
        res.json({
            message: `Bulk delete complete. Successfully deleted ${results.deleted.length} files.`,
            results
        });
    }
    catch (error) {
        console.error('[Backup] Bulk delete error:', error);
        res.status(500).json({ message: error.message || 'Failed to perform bulk delete' });
    }
});
exports.deleteMultipleBackups = deleteMultipleBackups;
// ──────────────────────────────────────────────────────────────────────────────
