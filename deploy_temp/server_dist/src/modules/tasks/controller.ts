...
const updateTaskSchema = createTaskSchema.partial().extend({
    status: z.enum(['PLANNED', 'ASSIGNED', 'IN_PROGRESS', 'REVIEW', 'REVISION_REQUESTED', 'COMPLETED', 'REWORK']).optional(),
});
...
export const getStats = async (req: Request, res: Response) => {
    try {
        ...
        if (view && ['staff', 'department', 'client', 'status', 'staff_type'].includes(view as string)) {
        ...
        if (view === 'digital-marketing') {
            ...
            const dmStats = await taskService.getDigitalMarketingDashboardStats(
