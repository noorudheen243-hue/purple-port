...
const onboardSchema = z.object({
    ...
    role: z.any().optional(),
    status: z.enum(['ACTIVE', 'RELIEVED']).optional(),
...
export const listStaff = async (req: Request, res: Response) => {
    try {
        const includeHidden = req.user?.role === 'ADMIN' || req.user?.role === 'DEVELOPER_ADMIN';
        const includeRelieved = req.query.include_relieved === 'true';
        const staff = await teamService.listStaff(includeHidden, includeRelieved);
        res.json(staff);
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
