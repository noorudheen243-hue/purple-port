...
exports.getDigitalMarketingDashboardStats = exports.calculateDashboardAggregates = exports.clearActiveTasks = exports.wipeAllTaskData = exports.getDashboardStats = exports.getTaskStats = exports.deleteTask = exports.updateTask = exports.stopTimer = exports.startTimer = exports.getTaskById = exports.getTasks = exports.createTask = void 0;
...
const getTasks = (filters) => __awaiter(void 0, void 0, void 0, function* () {
    let whereClause = {};
    if (filters === null || filters === void 0 ? void 0 : filters.campaign_id)
        whereClause.campaign_id = filters.campaign_id;
    if (filters === null || filters === void 0 ? void 0 : filters.client_id)
        whereClause.client_id = filters.client_id;
    if (filters === null || filters === void 0 ? void 0 : filters.assignee_id)
    ...
    if (filters === null || filters === void 0 ? void 0 : filters.campaign_type)
        whereClause.campaign_type = filters.campaign_type;
    ...
    const isDashboardMode = filters.dashboard === 'true';
...
const updateTask = (id, data, userId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (data.status) {
        ...
        if (data.status === 'REWORK') {
            ...
            yield prisma_1.default.timeLog.updateMany({
...
const getDigitalMarketingDashboardStats = (month, year) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // 1. Total Active Clients (Usually regardless of period, but user might want active in that period? 
    // Generally "Active Clients" is a snapshot. But we'll stick to a snapshot of current active status.)
    const activeClients = yield prisma_1.default.client.count({
        where: { status: 'ACTIVE' }
    });
    // 2. Active Meta Campaigns (Unique campaigns active in the selected period)
    const activeMetaLogs = yield prisma_1.default.metaAdsLog.findMany({
        where: Object.assign({ status: 'ACTIVE' }, periodFilter),
    ...
    // 3. Assigned Creative Tasks (Outstanding tasks as of the END of the period or current?)
    // User wants "Assigned" which usually means current backlog.
    const assignedCreativeTasks = yield prisma_1.default.task.count({
        where: {
            department: 'CREATIVE',
            status: { not: 'COMPLETED' },
    ...
    // 4. Completed Creative Tasks (Specifically in the selected period)
    const completedCreativeTasks = yield prisma_1.default.task.count({
        where: {
            department: 'CREATIVE',
            status: 'COMPLETED',
    ...
    // 5. Pending Creative Tasks (Overdue as of the end of the period)
    const pendingCreativeTasks = yield prisma_1.default.task.count({
        where: {
            department: 'CREATIVE',
            status: { not: 'COMPLETED' },
            due_date: { lt: endOfPeriod }
        }
    });
    // 6. Charts Data (Keep DM focused for charts as it's a DM dashboard)
    // 6. Charts Data (DM only, within period)
    const campaignTypeStatsObj = {};
    const allDMTasks = yield prisma_1.default.task.findMany({
        where: Object.assign({ department: 'DIGITAL_MARKETING', status: { not: 'COMPLETED' } }, periodFilter),
    ...
    // Tasks by Staff (DM only, within period)
    const staffTasksRaw = yield prisma_1.default.task.groupBy({
        by: ['assignee_id'],
        where: Object.assign({ department: 'DIGITAL_MARKETING', status: { not: 'COMPLETED' }, assignee_id: { not: null } }, periodFilter),
        _count: { id: true }
    });
    const staffIds = staffTasksRaw.map(s => s.assignee_id).filter(Boolean);
    const staffMembers = yield prisma_1.default.user.findMany({ where: { id: { in: staffIds } }, select: { id: true, full_name: true } });
    const staffStats = staffTasksRaw.map(s => {
        const staff = staffMembers.find(m => m.id === s.assignee_id);
        return {
            name: (staff === null || staff === void 0 ? void 0 : staff.full_name) || 'Unknown',
            value: s._count.id
        };
    });
    // Tasks by Status (DM only, within period)
    const statusStatsRaw = yield prisma_1.default.task.groupBy({
        by: ['status'],
        where: Object.assign({ department: 'DIGITAL_MARKETING' }, periodFilter),
        _count: { id: true }
    });
    const statusStats = statusStatsRaw.map(s => ({ name: s.status, value: s._count.id }));
    return {
        activeClients, // Renamed from totalClients
        activeMetaCampaigns, // Renamed from activeCampaigns
        assignedCreativeTasks, // Renamed from tasksAssigned
        completedCreativeTasks, // Renamed from tasksCompleted
        pendingCreativeTasks, // Renamed from overdueTasks
        charts: {
            campaignTypeStats,
            staffStats,
            statusStats
...
exports.getDigitalMarketingDashboardStats = getDigitalMarketingDashboardStats;
