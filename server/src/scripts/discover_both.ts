...
async function scanSubnet(subnet: string) {
    ...
    for (let i = 1; i <= 254; i++) {
        const ip = `${subnet}.${i}`;
        promises.push(checkPort(ip, 4370, 300));
    }
    ...
    if (activeIps.length === 0) {
        console.log(`No devices found on port 4370 in subnet ${subnet}.x`);
        return;
    }

    console.log(`Found ${activeIps.length} potential devices in subnet ${subnet}.x:`, activeIps);
...
async function main() {
    ...
    await scanSubnet('192.168.29');
    await scanSubnet('192.168.0');
}
