...
exports.getCreativeDashboardStats = exports.getCreativeTeamMetrics = exports.getAttendanceStats = exports.getTeamPerformanceStats = exports.getManagerStats = void 0;
...
const getTeamPerformanceStats = (department, startDate, endDate) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const performanceData = staff.map(user => {
        ...
        const finalScore = Math.round((completionScore * 0.5) +
            (qualityScore * 0.3) +
            (efficiencyScore * 0.2));
        return {
            id: user.id,
            name: user.full_name,
            role: user.role,
            designation: ((_a = user.staffProfile) === null || _a === void 0 ? void 0 : _a.designation) || user.role.replace(/_/g, ' '),
...
const getCreativeTeamMetrics = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [totalTasks, completedTasks, wipTasks, reviewTasks, plannedTasks] = yield Promise.all([
        prisma_1.default.task.count({ where: { assignee_id: { in: creativeIds } } }),
        prisma_1.default.task.count({ where: { assignee_id: { in: creativeIds }, status: 'COMPLETED' } }),
        prisma_1.default.task.count({ where: { assignee_id: { in: creativeIds }, status: 'IN_PROGRESS' } }),
        prisma_1.default.task.count({ where: { assignee_id: { in: creativeIds }, status: 'REVIEW' } }),
        prisma_1.default.task.count({ where: { assignee_id: { in: creativeIds }, status: 'PLANNED' } })
    ]);
    return {
        total: totalTasks,
        completed: completedTasks,
...
const getCreativeDashboardStats = () => __awaiter(void 0, void 0, void 0, function* () {
    const now = new Date();
    const IST_OFFSET = 330 * 60 * 1000;
    const istNow = new Date(now.getTime() + IST_OFFSET);
    istNow.setUTCHours(0, 0, 0, 0);
    ...
    const dailyAssignedTasks = yield prisma_1.default.task.findMany({
        where: {
    ...
    const formatTasksResponse = dailyAssignedTasks.map((t, index) => {
        ...
        return ({
            ...
            client_name: ((_a = t.client) === null || _a === void 0 ? void 0 : _a.name) || 'Internal',
            task_type: t.type || 'Generic',
            assigned_by: ((_b = t.assigned_by) === null || _b === void 0 ? void 0 : _b.full_name) || 'System'
        });
    });
    // 3. Helper to calculate efficiency
    const calculateEfficiency = (startDate_1, ...args_1) => __awaiter(void 0, [startDate_1, ...args_1], void 0, function* (startDate, sortKey = 'efficiency') {
        const tasksPeriod = yield prisma_1.default.task.findMany({
            where: {
                assignee_id: { in: creativeIds },
    ...
    const dailyEfficiency = yield calculateEfficiency(today, 'completed');
    const weeklyEfficiency = yield calculateEfficiency(startWeek, 'completed');
    const monthlyEfficiency = yield calculateEfficiency(startMnth, 'efficiency');
    return {
...
exports.getCreativeDashboardStats = getCreativeDashboardStats;
