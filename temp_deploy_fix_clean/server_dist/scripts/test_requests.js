...
const service_2 = require("../modules/attendance/service");
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const staff = yield prisma.user.findFirst({
            ...
            const leave = yield service_1.LeaveService.applyLeave(staff.id, 'CASUAL', new Date(), new Date(), "Test Leave Reason");
            console.log(`[OK] Leave Created: ID ${leave.id}`);
            // 3. Simulate Regularisation Request
            console.log("\n[TEST] Staff requesting regularisation...");
            const reg = yield service_2.AttendanceService.requestRegularisation(staff.id, new Date(), 'MISSED_PUNCH_IN', "Forgot card - Test");
            ...
            notifications.forEach(n => {
                ...
                if (n.type === 'LEAVE_REQUEST' && n.message.includes(staff.full_name))
                    leaveNotifFound = true;
                if (n.type === 'REGULARISATION_REQUEST' && n.message.includes(staff.full_name))
                    regNotifFound = true;
            });
            if (leaveNotifFound)
                console.log("[PASS] Admin received Leave Notification");
            else
                console.error("[FAIL] Admin did NOT receive Leave Notification");
            if (regNotifFound)
                console.log("[PASS] Admin received Regularisation Notification");
            else
                console.error("[FAIL] Admin did NOT receive Regularisation Notification");
            // 5. Simulate Admin Approval
            console.log("\n[TEST] Admin approving Leave Request...");
            yield service_1.LeaveService.updateLeaveStatus(leave.id, admin.id, 'APPROVED');
            ...
            yield service_2.AttendanceService.updateRegularisationStatus(reg.id, admin.id, 'APPROVED');
            console.log("[OK] Regularisation Approved.");
            // 8. Verify Attendance Record for Regularisation
            // Refetch
            const allRecordsAfter = yield prisma.attendanceRecord.findMany({
                where: { user_id: staff.id },
                orderBy: { date: 'desc' }
            });
