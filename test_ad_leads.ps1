...
# Test script to fetch leads via Ad ID
$script = @"
...
async function run() {
    ...
    try {
        const res = await axios.get(META_GRAPH_URL + '/' + formattedAccountId + '/ads', {
            params: { access_token: acc.accessToken, fields: 'id,name', limit: 10 }
        });
        ...
        for (const ad of ads) {
            ...
            try {
                ...
                console.log('Failed:', e.response ? e.response.data.error.message : e.message);
            }
        }
    } catch(e) {
