...
const createTaskSchema = zod_1.z.object({
    ...
    department: zod_1.z.enum(['CREATIVE', 'DIGITAL_MARKETING']).default('CREATIVE'),
    campaign_type: zod_1.z.string().optional(),
});
const updateTaskSchema = createTaskSchema.partial().extend({
    status: zod_1.z.enum(['PLANNED', 'ASSIGNED', 'IN_PROGRESS', 'REVIEW', 'REVISION_REQUESTED', 'COMPLETED', 'REWORK']).optional(),
});
...
const getStats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (view && ['staff', 'department', 'client', 'status', 'staff_type'].includes(view)) {
        ...
        // Digital Marketing Dashboard Special Case
        if (view === 'digital-marketing') {
            const { month, year } = req.query;
            const dmStats = yield taskService.getDigitalMarketingDashboardStats(month ? parseInt(month) : undefined, year ? parseInt(year) : undefined);
            console.log('[DM-CONTROLLER-STATS]', JSON.stringify(dmStats));
            return res.json(dmStats);
        }
